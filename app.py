    ...
    while sniffer.WEBSOCKET is not None:
        ...
        if data == 'CMD::START':
            ...
            if any([sniffer.FILTERED_ACTIVITIES[category]['flag'] for category in sniffer.FILTERED_ACTIVITIES]):
                await sniffer.WEBSOCKET.send(f"FA0::{sniffer.find_attackers('TCP-SYN')}{sniffer.find_attackers('TCP-SYNACK')}{sniffer.find_attackers('ICMP-POD')}")
            else:
                await sniffer.WEBSOCKET.send('FA0::No attack detected yet. Try again later.')
        else:
...
async def WS_sender():
    ...
    while sniffer.WEBSOCKET is not None:
        ...
        await sniffer.WEBSOCKET.send(f"FLAG:{sniffer.FILTERED_ACTIVITIES['TCP-SYN']['flag']},{sniffer.FILTERED_ACTIVITIES['TCP-SYNACK']['flag']},{sniffer.FILTERED_ACTIVITIES['ICMP-POD']['flag']}")
